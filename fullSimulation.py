...
parser.add_option("--resampling_radius", type="float", default='200', # Ramesh
                  dest="RADIUS", help="Radius within which core is uniformly distributed.")
parser.add_option("--raise_observation", type="float", default='0.', # Ramesh
                  dest="OBSERVATION_LEVEL", help="Raise observation to make it 2837m from sea level")
...
print "resampling radius", options.RADIUS
print "raise observation level by", options.OBSERVATION_LEVEL
print ""
...
if options.GENERATOR.lower() == "nugen":
    ...
    if options.GENERATOR == 'corsika-icetop':
        tray.AddSegment(segments.GenerateIceTopShowers, "GenerateIceTopShowers",
                        ...
                        Files = infiles,
                        GCDFile = options.GCDFILE,
                        x=0, y=0, r=options.RADIUS*I3Units.m, # Ramesh 800-->200m, 400m, 600m
                        TankResponse=options.TOPRESPONSE,
                        RunID=options.RUNNUMBER,
                        RaiseObservationLevel=options.OBSERVATION_LEVEL*I3Units.m
                    )
    else:
...
# Ramesh, Just keep MCPrimary of frame that does not pass any trigger condition.
#         MCPrimary is required to calculate Ngenerated.
tray.Add('Keep', 'keep',
         keys=['MCPrimary'],
         If=lambda frame: len(frame['I3Triggers'])==0)
